...
public class AzureDevOpsTools
{
    ...
    public ICollection<AIFunction> GetTools()
    {
        ...
        [
            ...
            AIFunctionFactory.Create(
                async (string signalId, string? parentRecordId) =>
                {
                    try
                    {
                        var context = await GetPipelineContextAsync(signalId);
                        ...
                        var timeline = await context.BuildClient.GetBuildTimelineAsync(projectId, buildId)
                            ?? throw new InvalidOperationException($"Failed to retrieve timeline for build {buildId}.");

                        var allRecords = timeline.Records?.ToList() ?? [];

                        Guid? parentId = string.IsNullOrEmpty(parentRecordId)
                            ? null
                            : Guid.Parse(parentRecordId);
                        ...
                        return JsonSerializer.SerializeToElement(new { Records = children, Error = (string?)null });
                    }
                    catch (Exception ex)
                    {
                        return JsonSerializer.SerializeToElement(new { Records = Array.Empty<object>(), Error = ex.Message });
                    }
                },
                "get_pipeline_timeline_records",
                "List child timeline records for an Azure DevOps pipeline build. Pass a parentRecordId (GUID) to get children of a specific Phase/Job/Stage, or omit it to get top-level records. Returns each record's Id, Name, RecordType, Result, and LogId."),
        ];
    }
