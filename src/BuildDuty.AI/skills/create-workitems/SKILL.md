...
List orphaned analyses for this triage run — non-resolved analyses on triage signals that are not linked to any work item. If there are none, return early with zeros.

### 2. Load each orphaned analysis

For each orphaned entry, load the analysis by its signal ID and analysis ID.
