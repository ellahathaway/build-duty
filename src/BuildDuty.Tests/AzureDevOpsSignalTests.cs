...
public class AzureDevOpsPipelineSignalTests
{
    ...
    private class TestableAzureDevOpsCollector : AzureDevOpsSignalCollector
    {
        ...
        public TestableAzureDevOpsCollector(
            ...
            : base(MockConfigProvider(config), Substitute.For<IRemoteTokenProvider>(), storageProvider, new ReleaseBranchResolver())
        {
    ...
    private static IBuildDutyConfigProvider MockConfigProvider(AzureDevOpsConfig config)
    {
        var provider = Substitute.For<IBuildDutyConfigProvider>();
        provider.GetConfig().Returns(new BuildDutyConfig { Name = "test", AzureDevOps = config });
        return provider;
    }
